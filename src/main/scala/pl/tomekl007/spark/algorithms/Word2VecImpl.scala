...
object Word2VecImpl {
  ...
  def apply(sentence: String, runConfig: RunConfig): Vector = {
    ...
    val sentenceDf = InputCleanUtil.tokenizeAndStopWords(spark.parallelize(List(sentence), 2).map(InputBody(_)).toDF(), sqlContext)
    sentenceDf.show()
  ...
  //todo try n-gram https://en.wikipedia.org/wiki/N-gram
  ...
  def apply(dataFrame: DataFrame, runConfig: RunConfig): Word2VectWithModel = {
    ...
    val word2Vec = new Word2Vec()
      .setInputCol("words")
      .setOutputCol("result")
      .setVectorSize(runConfig.W2VVectSize) //todo test for 100, 200, 300 dim
